...
using Weave, Pkg, IJulia, InteractiveUtils

repo_directory = joinpath(@__DIR__,"..")
cssfile = joinpath(@__DIR__, "..", "templates", "skeleton_css.css")
latexfile = joinpath(@__DIR__, "..", "templates", "julia_tex.tpl")

function weave_file(folder,file,build_list=(:html,:notebook); kwargs...)
...
function weave_all(build_list=(:html,:notebook))
...
function weave_folder(folder, build_list=(:html,:notebook))
...
function tutorial_footer(folder=nothing, file=nothing; remove_homedir=true)
    ...
    ## Appendix
     This tutorial is part of the CUDATutorials.jl repository, found at: <https://github.com/JuliaGPU/CUDATutorials.jl>.
     For more information on doing GPU computing with Julia, check out <https://juliagpu.org/>.
    """)
    if folder !== nothing && file !== nothing
        ...
        ```
        using CUDATutorials
        CUDATutorials.weave_file("$folder","$file")
        ```
        """)
    end
